...
RTLD_DEFAULT = contek_mr._memray.RTLD_DEFAULT
RTLD_NOW = contek_mr._memray.RTLD_NOW
PAYLOAD = """
import atexit
...
def inject(debugger: str, pid: int, port: int, verbose: bool) -> str | None:
    ...
    injecter = pathlib.Path(contek_mr.__file__).parent / "_inject.abi3.so"
...
class AttachCommand:
    ...
    def run(self, args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
        ...
        destination: contek_mr.Destination
        if args.output:
            live_port = None
            destination = contek_mr.FileDestination(
            ...
            destination = contek_mr.SocketDestination(server_port=live_port)
